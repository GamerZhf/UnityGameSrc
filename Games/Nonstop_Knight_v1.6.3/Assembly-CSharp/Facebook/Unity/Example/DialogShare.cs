namespace Facebook.Unity.Example
{
    ...
    internal class DialogShare : MenuBase
    {
        ...
        protected override void GetGui()
        {
            ...
            {
                FacebookDelegate<IShareResult> callback = new FacebookDelegate<IShareResult>(this.HandleResult);
            ...
            {
                FB.ShareLink(new Uri("https://developers.facebook.com/"), "Link Share", "Look I'm sharing a link", new Uri("http://i.imgur.com/j4M7vCO.jpg"), new FacebookDelegate<IShareResult>(this.HandleResult));
            }
            ...
            {
                FB.ShareLink(new Uri(this.shareLink), this.shareTitle, this.shareDescription, new Uri(this.shareImage), new FacebookDelegate<IShareResult>(this.HandleResult));
            }
            GUI.enabled = enabled && (!Constants.IsEditor || (Constants.IsEditor && FB.IsLoggedIn));
            if (base.Button("Feed Share - No To"))
            {
                FB.FeedShare(string.Empty, new Uri("https://developers.facebook.com/"), "Test Title", "Test caption", "Test Description", new Uri("http://i.imgur.com/zkYlB.jpg"), string.Empty, new FacebookDelegate<IShareResult>(this.HandleResult));
            }
            ...
            {
                FB.FeedShare(this.feedTo, !string.IsNullOrEmpty(this.feedLink) ? new Uri(this.feedLink) : null, this.feedTitle, this.feedCaption, this.feedDescription, !string.IsNullOrEmpty(this.feedImage) ? new Uri(this.feedImage) : null, this.feedMediaSource, new FacebookDelegate<IShareResult>(this.HandleResult));
            }
            GUI.enabled = enabled;
        }
