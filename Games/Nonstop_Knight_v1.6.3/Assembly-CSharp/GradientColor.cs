...
[RequireComponent(typeof(Text)), AddComponentMenu("UI/ToJ Effects/Gradient Color", 1)]
        ...
        if (this.IsActive() && (verts.Count != 0))
        {
            if (this.gradientMode == GradientMode.Global)
            {
                ...
                for (int j = 0; j < verts.Count; j++)
                {
                    ...
                    {
                        Color newColor = Color.Lerp(this.firstColor, this.secondColor, (position.y - vertex.position.y) / num2);
                        vertex.color = this.CalculateColor((Color) vertex.color, newColor, this.colorMode);
                    }
                    else
                    {
                        Color color2 = Color.Lerp(this.firstColor, this.secondColor, (vertex.position.x - position.x) / num3);
