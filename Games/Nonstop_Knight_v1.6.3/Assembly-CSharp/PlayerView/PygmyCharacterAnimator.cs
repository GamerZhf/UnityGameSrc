namespace PlayerView
{
    ...
    public class PygmyCharacterAnimator : AbstractCharacterAnimator
    {
        protected override Dictionary<AbstractCharacterAnimator.Action, AbstractCharacterAnimator.ActionParameters> getActionClipLinks()
        {
            ...
            string[] names = new string[] { "Attack3", "Attack4" };
        ...
        private void onCharacterMeleeAttackStarted(CharacterInstance sourceCharacter, CharacterInstance targetCharacter)
        {
            ...
            {
                if (base.CharacterView.Character.Prefab == CharacterPrefab.PygmyMelee)
                {
                    base.startAction(AbstractCharacterAnimator.Action.ATTACK_MELEE, base.CharacterView.Character.getAttackDuration() * 1f, true, 0, -1f);
                }
                else
                {
        ...
        private void onCharacterRangedAttackStarted(CharacterInstance sourceCharacter, Vector3 targetWorldPt)
        {
            if (sourceCharacter == base.CharacterView.Character)
            {
                if (base.CharacterView.Character.Prefab == CharacterPrefab.PygmyCaster)
                {
                    base.startAction(AbstractCharacterAnimator.Action.ATTACK_MELEE, base.CharacterView.Character.getAttackDuration() * 0.5f, true, 0, -1f);
                }
                else
                {
