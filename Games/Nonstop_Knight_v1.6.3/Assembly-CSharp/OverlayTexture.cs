...
[DisallowMultipleComponent, AddComponentMenu("UI/ToJ Effects/Overlay Texture", 0x12), RequireComponent(typeof(Text))]
public class OverlayTexture : BaseVertexEffect, IMaterialModifier
{
    ...
    public virtual Material GetModifiedMaterial(Material baseMaterial)
    {
        ...
        material2.name = baseMaterial.name + " with OT";
        ...
        if (this.IsActive())
        {
            ...
            if (verts.Count != 0)
            {
                ...
                if (this.textureMode == TextureMode.Global)
                {
                    ...
                    {
                        vertex = verts[j];
                        vertex.uv1 = new Vector2(1f + ((vertex.position.x - position.x) / num4), 2f - ((position.y - vertex.position.y) / num3));
                        verts[j] = vertex;
                    }
                }
                else
                {
                    ...
                    {
                        vertex = verts[k];
                        vertex.uv1 = new Vector2((float) (1 + (((((k % 6) != 0) && ((k % 6) != 5)) && ((k % 6) != 4)) ? 1 : 0)), (float) (1 + (((((k % 6) != 2) && ((k % 6) != 3)) && ((k % 6) != 4)) ? 1 : 0)));
                        verts[k] = vertex;
                    }
                }
            }
