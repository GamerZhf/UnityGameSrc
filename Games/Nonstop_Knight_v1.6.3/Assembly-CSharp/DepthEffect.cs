...
[AddComponentMenu("UI/ToJ Effects/Depth Effect", 2), RequireComponent(typeof(Text))]
public class DepthEffect : BaseVertexEffect
{
    ...
    protected void ApplyShadowZeroAlloc(List<UIVertex> verts, Color32 color, int start, int end, float x, float y, float factor)
    {
        for (int i = start; i < end; i++)
        {
            ...
            {
                position.x -= (this.depthPerspectiveStrength.x * (((position.x - this.m_TopLeftPos.x) / this.m_OverallTextSize.x) - 0.5f)) * factor;
            }
            position.y += y * factor;
            if (this.depthPerspectiveStrength.y != 0f)
            {
                position.y += (this.depthPerspectiveStrength.y * (((this.m_TopLeftPos.y - position.y) / this.m_OverallTextSize.y) - 0.5f)) * factor;
            }
    ...
    public override void ModifyVertices(List<UIVertex> verts)
    {
        if (this.IsActive())
        {
            ...
            {
                range = verts.GetRange(verts.Count - (component.cachedTextGenerator.characterCountVisible * 6), component.cachedTextGenerator.characterCountVisible * 6);
            }
            else
            {
                range = verts;
            ...
            if (range.Count != 0)
            {
                if ((this.depthPerspectiveStrength.x != 0f) || (this.depthPerspectiveStrength.y != 0f))
                {
                    ...
                    this.m_OverallTextSize = new Vector2(this.m_BottomRightPos.x - this.m_TopLeftPos.x, this.m_TopLeftPos.y - this.m_BottomRightPos.y);
                }
                int start = 0;
                int num4 = 0;
                start = num4;
                num4 = range.Count;
                this.ApplyShadowZeroAlloc(range, this.effectColor, start, range.Count, this.effectDirectionAndDepth.x, this.effectDirectionAndDepth.y, 0.25f);
                start = num4;
                num4 = range.Count;
                this.ApplyShadowZeroAlloc(range, this.effectColor, start, range.Count, this.effectDirectionAndDepth.x, this.effectDirectionAndDepth.y, 0.5f);
                start = num4;
                num4 = range.Count;
                this.ApplyShadowZeroAlloc(range, this.effectColor, start, range.Count, this.effectDirectionAndDepth.x, this.effectDirectionAndDepth.y, 0.75f);
                start = num4;
                num4 = range.Count;
                this.ApplyShadowZeroAlloc(range, this.effectColor, start, range.Count, this.effectDirectionAndDepth.x, this.effectDirectionAndDepth.y, 1f);
    ...
    public Vector2 depthPerspectiveStrength
    {
        get
        {
            return this.m_DepthPerspectiveStrength;
        }
        set
        {
    ...
    public Vector2 effectDirectionAndDepth
    {
        get
        {
            return this.m_EffectDirectionAndDepth;
        }
        set
        {
    ...
    public bool onlyInitialCharactersGenerateDepth
    {
        get
        {
            return this.m_OnlyInitialCharactersGenerateDepth;
        }
        set
        {
