namespace Pathfinding.Voxels
{
    ...
    {
        private const ushort BorderReg = 0x8000;
        ...
        private const int ContourRegMask = 0xffff;
        ...
        private const int MaxLayers = 0xffff;
        private const int MaxRegions = 500;
        public float maxSlope = 30f;
        public int minRegionSize = 100;
        public const uint NotConnected = 0x3f;
        private const int RC_AREA_BORDER = 0x20000;
        private const int RC_BORDER_VERTEX = 0x10000;
        ...
        private const int VERTEX_BUCKET_COUNT = 0x1000;
                    ...
                    while (num17 < num18)
                    {
                        ...
                        else
                        {
                            ...
                            if ((num19 != 0) && ((num19 & 0x8000) != 0x8000))
                            {
                                ...
                                item.verts = ClaimIntArr(simplified.Count, false);
        ...
        public void BuildPolyMesh(VoxelContourSet cset, int nvp, out VoxelMesh mesh)
        {
            ...
            for (int i = 0; i < cset.conts.Count; i++)
            {
                ...
                if (contour2.nverts >= 3)
                {
                    ...
                    num3 = Math.Max(num3, contour5.nverts);
                }
            }
            if (num >= 0xfffe)
            {
            ...
            Int3[] numArray = new Int3[num];
            int[] array = new int[num2 * nvp];
            int[] src = new int[num2];
            ...
            for (int j = 0; j < cset.conts.Count; j++)
            {
                ...
                if (contour.nverts >= 3)
                {
                    ...
                    int num10 = this.Triangulate(contour.nverts, contour.verts, ref indices, ref tris);
                    int num11 = index;
                    for (int n = 0; n < (num10 * 3); n++)
                    {
                        array[num6] = tris[n] + num11;
                        num6++;
                    }
                    for (int num13 = 0; num13 < num10; num13++)
                    {
                        src[num7] = contour.area;
                        num7++;
                    }
                    for (int num14 = 0; num14 < contour.nverts; num14++)
                    {
                        numArray[index] = new Int3(contour.verts[num14 * 4], contour.verts[(num14 * 4) + 1], contour.verts[(num14 * 4) + 2]);
                        index++;
                    }
                }
            }
            mesh = new VoxelMesh();
            Int3[] numArray6 = new Int3[index];
            for (int k = 0; k < index; k++)
            {
                numArray6[k] = numArray[k];
            }
            int[] dst = new int[num6];
            int[] numArray8 = new int[num7];
            Buffer.BlockCopy(array, 0, dst, 0, num6 * 4);
            Buffer.BlockCopy(src, 0, numArray8, 0, num7 * 4);
            mesh.verts = numArray6;
            mesh.tris = dst;
            mesh.areas = numArray8;
        }
                        ...
                        for (int k = 0; k < 4; k++)
                        {
                            ...
                            if ((((num8 >= 0) && (num9 >= 0)) && (num9 < num)) && (num8 < this.voxelArea.width))
                            {
                                ...
                                while (num10 < num11)
                                {
                                    ...
                                    if (((Math.Min((int) (span.y + span.h), (int) (span2.y + span2.h)) - num12) >= this.voxelWalkableHeight) && (Math.Abs((int) (span2.y - span.y)) <= this.voxelWalkableClimb))
                                    {
        ...
        {
            return (this.voxelOffset + new Vector3((x + 0.5f) * this.cellSize, this.voxelArea.compactSpans[i].y * this.cellHeight, (z + 0.5f) * this.cellSize));
        }
        ...
        public void DebugDrawCompactSpans()
        {
            ...
            for (int i = 0; num4 < num3; i++)
            {
                for (int j = 0; j < this.voxelArea.width; j++)
                {
                    Vector3 vector2 = ((Vector3) (new Vector3((float) j, 0f, (float) i) * this.cellSize)) + min;
                    CompactVoxelCell cell = this.voxelArea.compactCells[j + num4];
                    int num7 = (int) cell.index;
                    int num8 = (int) (cell.index + cell.count);
                    while (num7 < num8)
                    {
                        CompactVoxelSpan span = this.voxelArea.compactSpans[num7];
                        vector2.y = ((span.y + 0.1f) * this.cellHeight) + min.y;
                        topVerts[index] = vector2;
                        vector2.y = (span.y * this.cellHeight) + min.y;
                        bottomVerts[index] = vector2;
            ...
            for (int i = 0; num2 < num; i++)
            {
                for (int j = 0; j < this.voxelArea.width; j++)
                {
                    for (int k = num2 + j; (k != -1) && (linkedSpans[k].bottom != uint.MaxValue); k = linkedSpans[k].next)
                    {
                        ...
                        {
                            Debug.Log(top + " " + num7);
                            Debug.DrawLine(new Vector3(j * this.cellSize, top * this.cellHeight, i * this.cellSize) + min, new Vector3(j * this.cellSize, num7 * this.cellHeight, i * this.cellSize) + min, Color.yellow, 1f);
                        }
                        if ((num7 - top) < this.voxelWalkableHeight)
                        {
                        }
                    }
                }
        ...
        private void DrawLine(int a, int b, int[] indices, int[] verts, Color col)
        {
            ...
            Debug.DrawLine(this.VoxelToWorld(verts[index], verts[index + 1], verts[index + 2]), this.VoxelToWorld(verts[num2], verts[num2 + 1], verts[num2 + 2]), col);
        }
        ...
        public void FilterSmallRegions(ushort[] reg, int minRegionSize, int maxRegions)
        {
            ...
            bool flag = !object.ReferenceEquals(root, null) && (this.relevantGraphSurfaceMode != RecastGraph.RelevantGraphSurfaceMode.DoNotRequire);
            if (flag || (minRegionSize > 0))
            {
                ...
                if (flag)
                {
                    while (!object.ReferenceEquals(root, null))
                    {
                        int num6;
                        int num7;
            ...
            for (int i = 0; i < st1.Count; i++)
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    if (span.GetConnection(j) != 0x3fL)
                    {
                        ...
                        {
                            regs[index] = reg;
                            st1.Add(new Int3(num7, index, num8));
                        }
                    }
                }
            }
                ...
                else
                {
                    ...
                    for (int k = 0; k < 4; k++)
                    {
                        if (span.GetConnection(k) != 0x3fL)
                        {
                            ...
                            int num20 = num5 + this.voxelArea.DirectionZ[k];
                            int num21 = ((int) this.voxelArea.compactCells[item + num20].index) + span.GetConnection(k);
                            if ((this.voxelArea.areaTypes[num21] == num) && ((this.voxelArea.dist[num21] >= num2) && (srcReg[num21] == 0)))
                            {
                                srcReg[num21] = r;
                                srcDist[num21] = 0;
                                stack.Add(item);
        ...
        public void SimplifyContour(List<int> verts, List<int> simplified, float maxError, int maxEdgeLenght, int buildFlags)
        {
            ...
            while (num18 < (simplified.Count / 4))
            {
                ...
                else
                {
                    ...
                    Utility.Swap(ref num21, ref num24);
                }
                if (((verts[(num28 * 4) + 3] & 0xffff) == 0) || ((verts[(num28 * 4) + 3] & 0x20000) == 0x20000))
                {
                    while (num28 != num30)
                    {
                        float num31 = VectorMath.SqrDistancePointSegmentApproximate(verts[num28 * 4], verts[(num28 * 4) + 2] / this.voxelArea.width, a, num21 / this.voxelArea.width, b, num24 / this.voxelArea.width);
                        if (num31 > num26)
                        {
                            num26 = num31;
                            num27 = num28;
            ...
            if ((num34 > 0f) && ((buildFlags & 3) != 0))
            {
                ...
                while (num35 < (simplified.Count / 4))
                {
                    ...
                    if (num43 != -1)
                    {
                        ...
                        int num48 = simplified.Count / 4;
                        for (int n = num48 - 1; n > num35; n--)
                        {
            ...
            {
                int num51 = (simplified[(j * 4) + 3] + 1) % num17;
                int num52 = simplified[(j * 4) + 3];
                simplified[(j * 4) + 3] = (verts[(num51 * 4) + 3] & 0xffff) | (verts[(num52 * 4) + 3] & 0x10000);
            }
        }
        ...
        private int Triangulate(int n, int[] verts, ref int[] indices, ref int[] tris)
        {
            ...
            while (n > 3)
            {
                ...
                {
                    int num9 = Next(k, n);
                    if ((indices[num9] & 0x40000000) != 0)
                    {
                        int num10 = (indices[k] & 0xfffffff) * 4;
                        int num11 = (indices[Next(num9, n)] & 0xfffffff) * 4;
                        int num12 = verts[num11] - verts[num10];
                        int num13 = verts[num11 + 2] - verts[num10 + 2];
                        int num14 = (num12 * num12) + (num13 * num13);
                        if ((num6 < 0) || (num14 < num6))
                        {
                            num6 = num14;
                            num7 = k;
                        }
                ...
                int num15 = num7;
                int num16 = Next(num15, n);
                int num17 = Next(num16, n);
                numArray[index] = indices[num15] & 0xfffffff;
                index++;
                numArray[index] = indices[num16] & 0xfffffff;
                index++;
                numArray[index] = indices[num17] & 0xfffffff;
                index++;
                num++;
                n--;
                for (int m = num16; m < n; m++)
                {
                ...
                num15 = Prev(num16, n);
                if (Diagonal(Prev(num15, n), num16, n, verts, indices))
                {
                    indices[num15] |= 0x40000000;
                }
                else
                {
                    indices[num15] &= 0xfffffff;
                }
                if (Diagonal(num15, Next(num16, n), n, verts, indices))
                {
                    indices[num16] |= 0x40000000;
                }
                else
                {
                    indices[num16] &= 0xfffffff;
                }
            }
        ...
        public void VoxelizeInput()
        {
            ...
            float num4 = Mathf.Cos(Mathf.Atan(Mathf.Tan(this.maxSlope * 0.01745329f) * (y * this.cellSize)));
            float[] a = new float[9];
            ...
            Vector3[] vectorArray = new Vector3[num5];
            Matrix4x4 matrixx = (Matrix4x4.TRS(-new Vector3(0.5f, 0f, 0.5f), Quaternion.identity, Vector3.one) * Matrix4x4.Scale(new Vector3(x, y, x))) * Matrix4x4.TRS(-min, Quaternion.identity, Vector3.one);
            for (int j = 0; j < this.inputMeshes.Count; j++)
            {
                RasterizationMesh mesh = this.inputMeshes[j];
                ...
                {
                    Vector3 v = vectorArray[triangles[m]];
                    Vector3 vector3 = vectorArray[triangles[m + 1]];
                    Vector3 vector4 = vectorArray[triangles[m + 2]];
                    if (flag)
                    {
                        Vector3 vector5 = v;
                        v = vector4;
                    ...
                    int num12 = (int) Utility.Min(v.x, vector3.x, vector4.x);
                    int num13 = (int) Utility.Min(v.z, vector3.z, vector4.z);
                    int num14 = (int) Math.Ceiling((double) Utility.Max(v.x, vector3.x, vector4.x));
                    int num15 = (int) Math.Ceiling((double) Utility.Max(v.z, vector3.z, vector4.z));
                    num12 = Mathf.Clamp(num12, 0, this.voxelArea.width - 1);
                    num14 = Mathf.Clamp(num14, 0, this.voxelArea.width - 1);
                    num13 = Mathf.Clamp(num13, 0, this.voxelArea.depth - 1);
                    num15 = Mathf.Clamp(num15, 0, this.voxelArea.depth - 1);
                    if ((((num12 < this.voxelArea.width) && (num13 < this.voxelArea.depth)) && (num14 > 0)) && (num15 > 0))
                    {
                        int num16;
                        if (Vector3.Dot(Vector3.Cross(vector3 - v, vector4 - v).normalized, Vector3.up) < num4)
                        {
                            num16 = 0;
                        }
                        else
                        ...
                        Utility.CopyVector(a, 0, v);
                        Utility.CopyVector(a, 3, vector3);
                        Utility.CopyVector(a, 6, vector4);
                        for (int n = num12; n <= num14; n++)
                        {
                            int num19 = this.clipper.ClipPolygon(a, 3, vOut, 1f, -n + 0.5f, 0);
                            if (num19 >= 3)
                            {
                                num19 = this.clipper.ClipPolygon(vOut, num19, numArray3, -1f, n + 0.5f, 0);
                                if (num19 >= 3)
                                {
                                    float num20 = numArray3[2];
                                    float num21 = numArray3[2];
                                    for (int num22 = 1; num22 < num19; num22++)
                                    {
                                        float num23 = numArray3[(num22 * 3) + 2];
                                        num20 = Math.Min(num20, num23);
                                        num21 = Math.Max(num21, num23);
                                    }
                                    int num24 = Mathf.Clamp((int) Math.Round((double) num20), 0, this.voxelArea.depth - 1);
                                    int num25 = Mathf.Clamp((int) Math.Round((double) num21), 0, this.voxelArea.depth - 1);
                                    for (int num26 = num24; num26 <= num25; num26++)
                                    {
                                        int num27 = this.clipper.ClipPolygon(numArray3, num19, numArray4, 1f, -num26 + 0.5f, 2);
                                        if (num27 >= 3)
                                        {
                                            num27 = this.clipper.ClipPolygonY(numArray4, num27, numArray5, -1f, num26 + 0.5f, 2);
                                            if (num27 >= 3)
                                            {
                                                float num28 = numArray5[1];
                                                float num29 = numArray5[1];
                                                for (int num30 = 1; num30 < num27; num30++)
                                                {
                                                    float num31 = numArray5[(num30 * 3) + 1];
                                                    num28 = Math.Min(num28, num31);
                                                    num29 = Math.Max(num29, num31);
                                                }
                                                int num32 = (int) Math.Ceiling((double) num29);
                                                if ((num32 >= 0) && (num28 <= num))
                                                {
                                                    int num33 = Math.Max(0, (int) num28);
                                                    num32 = Math.Max(num33 + 1, num32);
                                                    this.voxelArea.AddLinkedSpan((num26 * this.voxelArea.width) + n, (uint) num33, (uint) num32, num16, this.voxelWalkableClimb);
                                                }
                                            }
                                        }
                                    }
        ...
        {
            Int3 num = voxelPosition * 0x3e8;
            int introduced4 = Mathf.RoundToInt(num.x * this.cellScale.x);
            int introduced5 = Mathf.RoundToInt(num.y * this.cellScale.y);
            num = new Int3(introduced4, introduced5, Mathf.RoundToInt(num.z * this.cellScale.z));
            return (num + ((Int3) this.voxelOffset));
        }
