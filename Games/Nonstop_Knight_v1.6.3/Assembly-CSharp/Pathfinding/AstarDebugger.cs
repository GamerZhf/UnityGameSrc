namespace Pathfinding
{
    ...
    [ExecuteInEditMode, HelpURL("http://arongranberg.com/astar/docs/class_pathfinding_1_1_astar_debugger.php"), AddComponentMenu("Pathfinding/Debugger")]
    public class AstarDebugger : MonoBehaviour
    {
        [CompilerGenerated]
        private static Func<int> <>f__am$cache24;
        [CompilerGenerated]
        private static Func<int> <>f__am$cache25;
        ...
        {
            PathTypeDebug[] debugArray1 = new PathTypeDebug[1];
            if (<>f__am$cache24 == null)
            {
                <>f__am$cache24 = new Func<int>(AstarDebugger.<debugTypes>m__C);
            }
            if (<>f__am$cache25 == null)
            {
                <>f__am$cache25 = new Func<int>(AstarDebugger.<debugTypes>m__D);
            }
            debugArray1[0] = new PathTypeDebug("ABPath", <>f__am$cache24, <>f__am$cache25);
            this.debugTypes = debugArray1;
        }

        [CompilerGenerated]
        private static int <debugTypes>m__C()
        {
            return PathPool.GetSize(typeof(ABPath));
        }

        [CompilerGenerated]
        private static int <debugTypes>m__D()
        {
            return PathPool.GetTotalCreated(typeof(ABPath));
        }
        ...
        public void Update()
        {
            if (this.show && (Application.isPlaying || this.showInEditor))
            {
                ...
                if (Application.isPlaying)
                {
                    ...
                    this.graph[index].fps = (Time.deltaTime >= Mathf.Epsilon) ? (1f / Time.deltaTime) : 0f;
